...
import { coalesce, groupBy } from './utils'
...
export class RipgrepTextSearchEngine implements TextSearchProvider {
  ...
  provideTextSearchResults(query: TextSearchQuery, options: TextSearchOptions, progress: Progress<ITextSearchResult>, token: CancellationToken): Promise<ISerializedSearchSuccess> {
    ...
    return new Promise((resolve, reject) => {
      ...
      ripgrepParser.on('result', (match: ITextSearchResult) => {
        gotResult = true;
        dataWithoutResult = '';
...
export class RipgrepParser extends EventEmitter {
  ...
  override on(event: 'result', listener: (result: ITextSearchResult) => void): this;
  ...
  private handleLine(outputLine: string): void {
    ...
    if (parsedLine.type === 'match') {
      ...
      const filePath = path.join(this.rootFolder, matchPath);
      const result = this.createTextSearchMatch(parsedLine.data, filePath);
      ...
      const filePath = path.join(this.rootFolder, contextPath);
      const result = this.createTextSearchContext(parsedLine.data, filePath);
      result.forEach(r => this.onResult(r));
    }
  }

  private createTextSearchMatch(data: IRgMatch, filePath: string): TextSearchMatch {
    ...
    const ranges: ISearchRange[] = coalesce(data.submatches.map((match, i) => {
      if (this.hitLimit) {
        return null;
      }
      ...
      const startColumn = inBetweenStats.numLines > 0 ?
      ...
      const endColumn = stats.numLines > 0 ?
        stats.lastLineLength :
        stats.lastLineLength + startColumn;

      prevMatchEnd = match.end;
      prevMatchEndCol = endColumn;
      prevMatchEndLine = endLineNumber;

      return { startLineNumber, startColumn, endLineNumber, endColumn, }
    }));

    return createTextSearchResult(filePath, fullText, ranges, this.previewOptions);
  }

  private createTextSearchContext(data: IRgMatch, path: string): TextSearchContext[] {
  ...
  private onResult(match: ITextSearchResult): void {
